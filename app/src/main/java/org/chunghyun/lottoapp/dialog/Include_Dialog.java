...
public class Include_Dialog extends Dialog {
    ...
    public Include_Dialog(@NonNull Context context, ArrayList<String> prev, ArrayList<String> exclude) {
        super(context);
    ...
    //리스너 초기화
    public void setDialogListener(Include_Dialog.DialogClickListener dialogListener){
        ...
        for(int i=1; i<=45; i++){
            ...
            textView[i-1].setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if(isClick[temp]) {
                        if(exclude.contains(temp+1 + "")){
                            Toast.makeText(getContext(), "제외수에 선택된 번호 입니다.", Toast.LENGTH_SHORT).show();
                            isClick[temp] = false;
                        }else if(prev.contains(temp+1 + "")){
                            Toast.makeText(getContext(), "이전회차 포함수에 선택된 번호 입니다.", Toast.LENGTH_SHORT).show();
                            isClick[temp] = false;
                        }else {
                            if (selectNumber.size() + prev.size() < 5) {
                                if (temp < 10)
                                ...
                                int tmp = 5 - prev.size() - selectNumber.size();
                                if(selectNumber.size() < 5)
                                    Toast.makeText(getContext(), "현재 선택 가능한 번호의 수는 " + tmp + "입니다", Toast.LENGTH_SHORT).show();
                                else
